...
export class ProviderCenterServiceComponent implements OnInit {
    ...
    fetchProviderCenterServices() {
        ...
        this.sServices.getAllProviderCenterServices(this.centerId).subscribe({
            next: (res) => {
                ...
                this.uniqueServiceNames = Array.from(new Set(res.Data.map(x => x.ServiceName)));
                this.applyFilters();
                this.loading = false;
            },
            error: () => {
    ...
    applyFilters() {
        this.filteredProviderCenterServices = this.providerCenterServices.filter(pcs => {
            const matchesProvider = this.searchProvider.trim() === '' || pcs.ProviderName.toLowerCase().includes(this.searchProvider.trim().toLowerCase());
            const matchesService = this.filterService === '' || pcs.ServiceName === this.filterService;
    ...
    updatePaginatedProviderCenterServices() {
        ...
        this.paginatedProviderCenterServices = this.filteredProviderCenterServices.slice(startIndex, endIndex);
    }

    get totalPages(): number {
        return Math.ceil(this.filteredProviderCenterServices.length / this.pageSize);
    }
    ...
    saveEdit() {
        ...
        this.sServices.updateProviderCenterService(this.selectedPCS.Id, { Price: this.editPrice }).subscribe({
            next: () => {
                ...
                this.messageService.add({
                    key: 'main-toast',
                    severity: 'success', summary: 'Success', detail: 'Price updated successfully!'
                });
            },
            error: () => {
                console.log(this.editPrice);
                ...
                this.messageService.add({
                    key: 'main-toast',
                    severity: 'error', summary: 'Error', detail: 'Failed to update price.'
                });
            }
        });
    }
